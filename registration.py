...
class Graph:
    ...
    def has_cycle(self):
        ...
        depth_copy = [vertex.depth for vertex in self.vertices]

        for _ in range(len(self.vertices)):
            found = False
            for i in range(len(depth_copy)):
                if depth_copy[i] == 0:
                    found = True
                    depth_copy[i] = -1
                    for neighbor in self.get_adjacent_vertices(i):
                        depth_copy[neighbor] -= 1
                    break
            if not found:
                return True
        return False
    ...
    def get_registration_plan(self):
        ...
        while visited_count < total_vertices:
            semester = []
            for vertex in self.vertices:
                if not vertex.visited and vertex.depth == 0:
                    ...
                    for neighbor_index in range(len(self.vertices)):
                        if self.adjacency_matrix[vertex_index][neighbor_index] == 1:
                            self.vertices[neighbor_index].depth -= 1
