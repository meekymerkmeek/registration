...
class Graph:
    ...
    def compute_depth(self):
        ...
        while queue:
            ...
            for neighbor in self.get_adjacent_vertices(current):
                self.vertices[neighbor].depth = max(self.vertices[neighbor].depth, self.vertices[current].depth + 1)
    ...
    def has_cycle(self):
        ...
        temp_depth = [0] * len(self.vertices)

        for j, _ in enumerate(self.vertices):
            for i, _ in enumerate(self.vertices):
                if self.adjacency_matrix[i][j]:
                    temp_depth[j] += 1

        queue = [i for i, deg in enumerate(temp_depth) if deg == 0]
        visited = 0
        ...
        while queue:
            ...
            visited += 1
            for neighbor in self.get_adjacent_vertices(current):
                temp_depth[neighbor] -= 1
                if temp_depth[neighbor] == 0:
                    queue.append(neighbor)
    ...
    def get_registration_plan(self):
        ...
        total_courses = len(self.vertices)
        taken_courses = 0

        while taken_courses < total_courses:
            semester = []
